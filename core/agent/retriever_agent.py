...
def prepare_knowledge(schema: Dict[str, Any], doc_type: str, enable_cmp_sql: bool = False) -> str:
    """
...
"""
    ...
    if enable_cmp_sql and doc_type == "screenplay":
        sql_cols = ["名称", "类别", "子类别", "外观", "状态", "相关角色", "文中线索", "补充信息", "chunk_id", "场次", "场次名", "子场次名"]
        col_txt = ", ".join(sql_cols)
        full_knowledge += f"\n另外，服饰、化妆、道具的信息保存在 SQL 数据库中，表名为 CMP_info，有以下一些 column name：\n{col_txt}\n"
...
class QuestionAnsweringAgent:
    ...
    def _build_knowledge(self, mode: Mode) -> str:
        if mode == "graph_only":
            ...
            return prepare_knowledge(schema, self.doc_type, self.enable_cmp_sql) if schema else ""
        elif mode == "vector_only":
            return ""
        else:  # hybrid
            parts = []
            schema = self._load_graph_schema()
            if schema:
                parts.append(prepare_knowledge(schema, self.doc_type, self.enable_cmp_sql))
    ...
    def _build_native_tools(self, *, doc_path: str = None) -> List[Any]:
        ...
        tools = [BM25SearchDocsTool(documents)]
        if self.enable_cmp_sql and self.db_path:
            tools.extend([
